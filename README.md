...
WARDEN / TIDEBREAKER / COILHEAD / ASHMAW / NULLPRIEST / GRAVELOCK / SPARKWIDOW / OBSIDIAN CROWN の8体。装甲は72から132、体格も所持技も別々で、8種の攻撃（扇状射撃・床の波・突進・曲射・全方位・跳躍・地雷・弾幕の壁）を体ごとに配分しています。さらに COILHEAD は**普段は宙を飛んでいて**（床から90px上を移動して空中から撃ち、突進は低く沈み込む滑空になる）、降りてくるのは**急降下の毒針**の時だけ（真上から狙い、落ちる直前に位置を固定して床に刺さり、左右に電撃を走らせたあと約1秒動けない）、SPARKWIDOW は**糸渡りの丸鋸**（アリーナ中央の天井に糸を掛けて振り子のように床すれすれを横切り、着地すると往復する丸鋸を左右へ投げる）という固有技を持ちます。**各ボスは4〜6拍の決まった順番を繰り返します。** 1拍は「その技が要る距離まで歩く→溜める→撃つ→立ち止まって隙を晒す」で、隙は最短0.60秒あります。定義は [`assets/bosses.js`](assets/bosses.js) の `routine` にデータとして置いてあります。ループと隙の実測は `node qa/boss-motion.cjs` で出せます。
