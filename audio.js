...
var ac=null,master=.4,music=.45,sfx=.7,muted=false,paused=false,playing=false,resultMode=false,media=null,track="game",seGain;
function ensureMedia(){if(!media){media=new Audio(track==='title'?'assets/title-bgm.mp3':'assets/stage1-bgm.mp3');media.loop=true;media.preload='auto'}return media}
function selectTrack(next){if(track!==next&&media){media.pause();media=null}track=next;var m=ensureMedia();m.currentTime=0;playing=true;paused=false;resultMode=false;syncMedia();playMedia()}
