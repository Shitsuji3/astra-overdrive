...
test('every drawn state resolves to one rig stance',()=>{
 const {api}=harness();
 const base={x:100,y:270,w:24,h:40,vx:0,vy:0,onGround:true,facing:1,dashTime:0,wallDir:0,saberTime:0,shootPoseTime:0};
 ...
 // firing keeps the same stance instead of dropping to the old sheet
 assert.equal(api.rigMode({...base,shootPoseTime:.1}),'idle');
 assert.equal(api.rigMode({...base,vx:190,shootPoseTime:.1}),'run');
});

test('all original movement poses remain and air shooting aims forward',()=>{
 const {api}=harness(),base={onGround:true,shootPoseTime:0};
 for(const [change,frame] of [[{},0],[{vx:190,animTime:0},1],[{vx:190,animTime:.1},2],[{vx:190,animTime:.2},3],[{onGround:false},4],[{dashTime:.1},5],[{wallDir:1},6],[{saberTime:.1},7],[{onGround:false,shootPoseTime:.1},0]])assert.equal(api.frame({...base,...change}),frame,JSON.stringify(change));
});
for(const facing of [-1,1])for(const pose of ['standing','running','airborne','dash','landing'])test(`${pose} ${facing}: a shot starts at the rendered muzzle`,()=>{
 {
...
test('actual hits deal 1 / 4.5 to enemies and bosses, retaining fractions',()=>{
 ...
 for(const kind of ['normal','saber']){
  ...
  if(kind==='saber')events['canvas-mousedown']({button:2});else if(kind==='normal'){g._shootHeld=true;}
  ...
  const expected=kind==='normal'?1:4.5;
 ...
 assert.equal(damages.saber/damages.normal,4.5,'the saber is the main weapon');
});




test('stationary firing keeps the standing rig barrel',()=>{
 const {api,rig}=harness();
 for(const facing of [-1,1])for(const action of [{shootPoseTime:.1}]){
  const p={x:100,y:270,w:24,h:40,vx:0,vy:0,onGround:true,facing,animTime:0,...action};
  assert.equal(api.rigMode(p),'idle');
  const still={...p,shootPoseTime:0};
