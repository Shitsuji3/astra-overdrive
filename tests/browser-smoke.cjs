...
(async () => {
  ...
  try {
    ...
    const shotsBefore = await page.evaluate(() => game.state.shots);
    await page.keyboard.down('j'); await page.waitForTimeout(1300);
    checked('holding fire neither charges nor fires', await page.evaluate(n => !(game.state.player.charge > 0) && game.state.shots === n, shotsBefore));
    await page.keyboard.up('j');
    checked('releasing fire shoots once', await page.waitForFunction(n => game.state.shots === n + 1, shotsBefore).then(() => true));
    ...
    checked('death opens retry after breakup', await page.locator('#overlay').isVisible());
