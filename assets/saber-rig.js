(function (g) {
  ...
  var keys = [
    [
      [0, 0, -23, 0.02, 7, -27, -0.8, 8, -8],
      [0.1, -2, -23.5, -0.16, -6, -33, -1.1, 8, -10],
      [0.22, -3, -23, -0.24, -10, -39, -0.85, 9, -12],
      [0.32, 0, -21, 0.18, 8, -43, -1.75, 14, -17],
      [0.44, 1, -21, 0.2, 17, -36, -0.35, 13, -19],
      [0.56, 2, -20.5, 0.23, 22, -22, 0.55, 12, -21],
      [0.7, 2, -20.5, 0.25, 23, -14, 0.95, 12, -22],
      [0.88, 2, -20.7, 0.23, 22, -15, 0.82, 12, -22],
      [1, 4, -20, 0.24, 18, -20, 0.95, 18, -16]
    ],
    [
      [0, 4, -20, 0.24, 18, -20, 0.95, 18, -16],
      [0.12, 2, -19, 0.3, 20, -24, -0.2, 16, -17],
      [0.24, 3, -17.5, 0.34, 23, -22, 0.1, 21, -20],
      [0.38, 2, -16.5, 0.2, 17, -22, 0.9, 23, -22],
      [0.5, 1, -18.5, 0.04, -10, -25, 1.8, 20, -20],
      [0.62, 0, -21, -0.02, -22, -27, 2.7, 14, -14],
      [0.74, -1, -21.5, -0.04, -24, -28, 3.15, 13, -13],
      [0.86, -1, -21.8, 0, -20, -29, 3.3, 12, -12],
      [1, 0, -22, 0.04, 6, -29, -0.9, 12, -12]
    ],
    ...
    [
      [0, 0, -22, 0.04, 6, -29, -0.9, 12, -12],
      [0.08, -2, -23, -0.18, -6, -34, -1.1, 10, -13],
      [0.16, -4, -23.5, -0.34, -10, -39, -0.85, 11, -15],
      [0.23, 0, -21, 0.18, 8, -43, -2.0, 16, -18],
      [0.31, 1, -19, 0.34, 16, -40, -0.8, 19, -21],
      [0.41, 3, -18.5, 0.35, 20, -22, 0.55, 22, -21],
      [0.51, 3, -17.5, 0.4, 17, -13, 1.15, 23, -22],
      [0.92, 3, -17.8, 0.38, 17, -14, 0.9, 23, -22],
      [1, 0, -23, 0.025, 8, -27, 0.7, 8, -8]
    ],
    ...
    [
      [0, 0, -23, 0.02, 7, -27, -0.8, 8, -8],
      [0.08, 1, -21.5, 0.14, 13, -25, 0, 12, -11],
      [0.14, 2, -20, 0.22, 17, -24, 0, 15, -15],
      [0.22, 3, -18.5, 0.3, 20, -22, 0, 19, -19],
      [0.36, 4, -17, 0.38, 25, -21, 0, 22, -22],
      [0.62, 4, -17, 0.38, 25, -21, 0, 22, -22],
      [0.8, 3, -18, 0.32, 22, -22, 0, 21, -21],
      [0.9, 1, -21, 0.12, 12, -25, -0.4, 13, -13],
      [1, 0, -23, 0.02, 7, -27, -0.8, 8, -8]
    ]
  ];
  // Reference eight cells: rise, dome expansion, contraction, low release, recovery.
  keys.push([
    [0, 0, -23, 0.02, 7, -27, -0.8, 8, -8],
    [0.12, 0, -24, -0.08, 0, -45, -1.6, 8, -8],
    [0.22, 0, -25, -0.08, 0, -49, -1.6, 9, -9],
    [0.4, 0, -24, -0.04, 1, -48, -1.6, 10, -10],
    [0.55, 0, -21, 0.14, 7, -34, -1.2, 13, -13],
    [0.68, 2, -16, 0.4, 17, -13, -0.3, 20, -19],
    [0.78, 2, -16, 0.4, 18, -13, -0.3, 20, -19],
    [1, 0, -23, 0.02, 7, -27, -0.8, 8, -8]
  ]);
  ...
  function plant(x, footY, hip, extend) {
    var rise = footY - hip.y,
      ahead = x >= hip.x ? 1 : -1,
      span = Math.abs(x - hip.x);
    if (extend) {
  ...
  function pose(stage, t) {
    ...
    var a = list[i],
      b = list[i + 1],
      span = b[0] - a[0],
      raw = clamp((t - a[0]) / span, 0, 1),
      u = raw * raw * (3 - 2 * raw);
    var rate = (6 * raw * (1 - raw)) / span,
      leanRate = (b[3] - a[3]) * rate,
      driveRate = (b[1] - a[1]) * rate;
    var v = a.map(function (n, j) {
      return n + (b[j] - n) * u;
    });
    var hip = { x: v[1], y: v[2] },
      lean = v[3];
    ...
    function bodyPoint(x, y) {
      return {
        x: hip.x + x * Math.cos(lean) - y * Math.sin(lean),
        y: hip.y + x * Math.sin(lean) + y * Math.cos(lean)
      };
    }
    ...
    var dx = hand.x - shoulder.x,
      ...
      d = Math.hypot(dx, dy);
    if (d > 17) {
      hand = { x: shoulder.x + (dx * 17) / d, y: shoulder.y + (dy * 17) / d };
    }
    return {
      stage: stage,
      t: t,
      ...
      rearFoot: {
        x: plant(v[8], -4 - 1.5 * open - (v[10] || 0), hip, lunge),
        y: -4 - 1.5 * open - (v[10] || 0)
      },
      frontFootAngle: -0.1 * open * (1 - 0.65 * twist),
      rearFootAngle: 0.26 * open * (1 - 0.65 * twist)
    };
  }
  ...
  function joint(a, b, l1, l2, bend) {
    var dx = b.x - a.x,
      ...
      r = clamp(d, Math.abs(l1 - l2) + 0.01, l1 + l2 - 0.01);
    var along = (l1 * l1 - l2 * l2 + r * r) / (2 * r),
      h = Math.sqrt(Math.max(0, l1 * l1 - along * along));
    return {
      x: a.x + (dx / d) * along + (dy / d) * h * bend,
      y: a.y + (dy / d) * along - (dx / d) * h * bend
    };
  }
  ...
  function blade(p) {
    ...
    var t = p.t,
      ignite = p.stage === 3 ? 0.04 : 0.1,
      visible = t > ignite && t < 0.95;
    var growth =
      p.stage === 2
        ? ease(0.1, 0.23, t)
        : 0.25 * ease(ignite, p.stage === 3 ? 0.16 : 0.22, t) +
          0.75 * ease(p.stage === 3 ? 0.16 : 0.22, p.stage === 3 ? 0.31 : 0.44, t);
    ...
    var fade = p.stage === 2 ? 0.6 : 0.72;
    length *= t > fade ? Math.max(0.08, 1 - ease(fade, 0.96, t)) : 1;
    var x, y;
    if (p.stage === 2) {
      var a = p.angle;
      x = p.hip.x + 4 + Math.cos(a) * length - p.hand.x;
      y = p.hip.y - 8 + Math.sin(a) * length * 0.15 - p.hand.y;
    } else {
      x = Math.cos(p.angle) * length;
      y = Math.sin(p.angle) * length;
    }
    if (p.stage === 6) {
      var fx = thrustFx(p),
        grip = { x: p.hand.x, y: p.hand.y };
      ...
      if (fx.lance && fx.lance.broken < 0.5)
        return { root: grip, tip: { x: fx.lance.x + fx.lance.len, y: p.hand.y }, visible: true };
    ...
    if (p.stage >= 4) {
      ...
      return {
        root: f.root,
        tip: { x: f.root.x + Math.cos(f.angle) * f.length, y: f.root.y + Math.sin(f.angle) * f.length },
        visible: true
      };
    }
    return {
      ...
      tip: {
        x: p.hand.x + x,
        y: p.hand.y + y > -4 ? -4 + 6 * Math.tanh((p.hand.y + y + 4) / 6) : p.hand.y + y
      },
      visible: visible
    };
  }
  ...
  var smearKeys = [
    [
      [0.3, -22, -17, -12, -35, 1, -15, -7, -17, -15, -21],
      [0.44, -7, -47, 24, -44, 43, -11, 20, -17, 6, -38],
      [0.56, 6, -55, 45, -38, 53, 17, 31, 0, 25, -22],
      [0.7, 33, -27, 39, 20, 23, 22, 16, 10, 32, 9],
      [0.8, 35, -2, 30, 20, 10, 7, 10, 1, 24, 11]
    ],
    [
      [0.21, -31, -21, -20, -37, -5, -19, -12, -15, -23, -23],
      [0.41, -15, -53, 20, -55, 50, -20, 20, -5, 24, -33],
      [0.51, -10, -57, 45, -45, 55, 30, 37, 12, 35, -25],
      [0.6, 40, -32, 64, 27, 22, 32, 37, 12, 56, 2],
      [0.68, 53, 0, 50, 26, 23, 26, 24, 13, 43, 16],
      [0.78, 33, 18, 26, 26, 8, 17, 12, 12, 25, 20]
    ],
    ...
    [
      [0.18, 2, 26, 20, 22, 16, 2, 5, 9, 12, 19],
      [0.3, 28, 14, 36, -4, 28, -26, 15, -3, 28, -6],
      [0.42, 36, -20, 32, -42, 13, -50, 13, -17, 28, -34],
      [0.54, 15, -48, -3, -55, -17, -42, -2, -23, 7, -46],
      [0.66, -5, -46, -23, -36, -24, -19, -11, -17, -15, -34]
    ]
  ];
  function smear(p) {
    if (p.stage === 2 || p.stage === 6) return null;
    var list = smearKeys[p.stage === 4 ? 2 : p.stage === 3 ? 1 : 0],
      start = list[0][0],
      end = p.stage === 4 ? 0.76 : p.stage === 3 ? 0.82 : 0.84,
      from = p.stage === 4 ? 0.64 : p.stage === 3 ? 0.72 : 0.73;
    if (p.t < start || p.t >= end) return null;
    var i = 0;
    while (i < list.length - 2 && p.t > list[i + 1][0]) i++;
    var a = list[i],
      b = list[i + 1],
      u = ease(a[0], b[0], p.t);
    return {
      points: a.slice(1).map(function (n, j) {
        return n + (b[j + 1] - n) * u;
      }),
      alpha: ease(start, start + 0.02, p.t) * (1 - ease(from, end, p.t))
    };
  }
  ...
  function fireSide(u, side) {
    return fireLerp(side < 0 ? FIRE_UP : FIRE_DN, u);
  }
  // The mean of the two sides, which is what sizes the field.
  function fireHalf(u) {
    return 0.5 * (fireLerp(FIRE_UP, u) + fireLerp(FIRE_DN, u));
  }
  ...
  function fireNoise(x, y, step, scale) {
    ...
    fx = fx * fx * (3 - 2 * fx);
    fy = fy * fy * (3 - 2 * fy);
    var a = fireHash(ix * 1.7 + iy * 97.3, step),
      b = fireHash((ix + 1) * 1.7 + iy * 97.3, step),
      c = fireHash(ix * 1.7 + (iy + 1) * 97.3, step),
      d = fireHash((ix + 1) * 1.7 + (iy + 1) * 97.3, step);
  ...
  function fireHeat(px, y, u, hw, step, size) {
    ...
    var uc = Math.max(0, Math.min(1, u)),
      mean = Math.max(0.05, fireHalf(uc)),
    ...
    if (y > 0) side *= 1 + 0.85 * (fireNoise(px * 0.9 + 300, uc * 40, step + 13, 4.5) - 0.5);
    else side *= 1 + 0.34 * (fireNoise(px * 1.3 + 700, uc * 40, step + 29, 3.2) - 0.5);
    if (side < 0.6) return -1;
    var v = y / side;
    ...
    var heat =
      H.base +
      H.gain * (size === undefined ? 1 : size) * Math.pow(Math.min(1, Math.max(0, uc * H.reach)), H.curve) -
      H.lead * v -
      H.edge * Math.abs(v);
    ...
    var n = 0.62 * fireNoise(px, y, step, 6.5) + 0.38 * fireNoise(px * 1.7 + 40, y * 1.7, step + 31, 2.6);
    ...
    if (heat < FIRE[7].s && fireNoise(px * 2.6 + 11, y * 2.6, step + 77, 3.2) < 0.16) return -1;
    return heat;
  }
  ...
  function fireField(len, wide) {
    var half = Math.ceil(wide * 0.5 * 2.6) + 7;
  ...
  function forEachFireCell(len, wide, step, fn) {
    var size = Math.sqrt(Math.max(0.3, Math.min(1, len / FIRE_FULL))),
      F = fireField(len, wide);
    for (var px = 0; px < F.W; px++) {
      var u = (px - 4) / Math.max(1, len);
      if (u < -0.05 || u > 1.06) continue;
      var hw = wide * 0.5 * fireHalf(Math.max(0, Math.min(1, u)));
      if (hw < 0.6) continue;
      for (var py = 0; py < F.H; py++) {
        var y = py - F.half - 1,
          heat = fireHeat(px, y, u, hw, step, size);
        if (heat < 0) continue;
  ...
  function bakeFire(len, wide, step, reduced) {
    ...
    var c = cv.getContext('2d'),
      ...
      d = img.data;
    var rgb = FIRE.map(function (b) {
      var n = parseInt(b.c.slice(1), 16);
  ...
  function fireCells(stage, phase, build, reduced) {
    var q = pose(stage, phase),
      f = stage === 4 || stage === 5 ? plume(q) : null,
      out = [];
    if (!f || f.alpha < FIRE_BITES_FROM) return out;
    var len = Math.round(f.length),
      wide = Math.round(f.width),
      step = reduced ? 0 : Math.floor(q.t * 34) % 8;
    var bx = (build && build.x) || 1,
      by = (build && build.y) || 1,
      c = Math.cos(f.angle),
      s = Math.sin(f.angle);
  ...
  function fireSlices(stage, phase, build, reach) {
    var q = pose(stage, phase),
      ...
      out = [];
    if (!f || f.alpha < FIRE_BITES_FROM) return out;
    var r = reach === undefined ? FIRE_REACH : reach,
      len = Math.round(f.length),
      w = Math.round(f.width) * 0.5 * r;
    var bx = (build && build.x) || 1,
      by = (build && build.y) || 1,
      c = Math.cos(f.angle),
      s = Math.sin(f.angle);
    function at(x, y) {
      return { x: (f.root.x + x * c - y * s) * bx, y: (f.root.y + x * s + y * c) * by };
    }
    function lead(u) {
      return at(u * len, -w * fireSide(u, -1));
    }
    function trail(u) {
      return at(u * len, w * fireSide(u, 1));
    }
    ...
    for (var k = 0; k <= FIRE_SLICES; k++) {
      ...
      if (k < FIRE_SLICES) {
        var v = (k + 1) / FIRE_SLICES;
        out.push({ a: lead(u), b: lead(v) });
        out.push({ a: trail(u), b: trail(v) });
      }
    }
    return out;
  }
  function drawPlume(ctx, p, reduced) {
    ...
    ctx.save();
    ctx.translate(f.root.x, f.root.y);
    ctx.rotate(f.angle);
    ...
    if (!reduced) {
      ...
      for (var e = 0; e < 12; e++) {
        var e1 = fireHash(e * 13.1, churn),
          e2 = fireHash(e * 17.7 + 80, churn),
          e3 = fireHash(e * 5.3 + 160, churn);
        var trailing = e % 4 !== 3,
          uu = 0.18 + 0.74 * e1,
          edge = f.width * 0.5 * fireSide(uu, trailing ? 1 : -1);
        ctx.fillStyle = DRIP[Math.floor(e3 * DRIP.length)];
        ctx.fillRect(
          Math.round(f.length * uu),
          Math.round(trailing ? edge * (1.05 + 0.55 * e2) : -edge * (1.05 + 0.35 * e2)),
          1 + Math.round(e3),
          1 + Math.round(2 * e2)
        );
      }
  ...
  function trail(ctx, p, reduced) {
    ...
    if (v[5] > 0) v[5] *= 1.65;
    ctx.save();
    ctx.translate(p.hand.x, p.hand.y);
    function crescent(scale, color, alpha) {
      ...
      ctx.closePath();
      ctx.fillStyle = color;
      ctx.globalAlpha *= shape.alpha * alpha;
      ctx.fill();
      ctx.restore();
    }
    if (!reduced) crescent(1.055, '#15964f', 0.3);
    crescent(1, '#2dc775', 1);
    crescent(0.965, '#a0f5c3', 1);
    crescent(0.91, '#f4fff9', 1);
    ctx.restore();
  }
  function horizontalTrail(ctx, p, front) {
    if (p.t < 0.23 || p.t > 0.64) return;
    var cx = p.hip.x + 4,
      cy = p.hip.y - 8,
      fade = Math.min(1, (p.t - 0.23) / 0.1) * (1 - ease(0.45, 0.64, p.t));
    var radius = 48,
      head = p.angle,
      span = Math.min(3.8, Math.max(0.1, 0.8 + head * 2.2));
    ctx.save();
    ctx.beginPath();
    ctx.rect(cx - 65, front ? cy : cy - 24, 130, 24);
    ctx.clip();
    function ribbon(width, color, opacity) {
      ...
      for (var i = 0; i <= 36; i++) {
        var u = i / 36,
          a = head - span * u,
          w = width * Math.pow(Math.sin(Math.PI * u), 0.6);
        var x = cx + radius * Math.cos(a),
          y = cy + radius * 0.15 * Math.sin(a);
        if (i === 0) ctx.moveTo(x, y);
        else ctx.lineTo(x, y);
        inside.push({ x: cx + (radius - w) * Math.cos(a), y: cy + (radius * 0.15 - w * 0.32) * Math.sin(a) });
      }
    ...
    ribbon(11, '#30bc70', 0.8);
    ctx.globalAlpha = alpha;
    ribbon(8, '#9cfcc5', 0.95);
    ctx.globalAlpha = alpha;
    ribbon(5.5, '#f1fff7', 1);
    ctx.restore();
  }
  ...
  var THRUST_SCARF = -0.16;
  // dark rim, blue, pale blue, white
  var THRUST_INK = ['#2058d8', '#40a0f8', '#a8d8f8', '#f8f8f8'];
  function thrustFx(p) {
    var t = p.t,
      h = p.hand,
      out = { orb: null, lance: null, arcs: 0 };
    if (t < 0.12) {
      var r = 1.5 + 3.5 * ease(0, 0.1, t);
      out.orb = { x: h.x + r * 0.9, y: h.y, rx: r, ry: r, ring: false };
      out.arcs = ease(0.05, 0.12, t);
    } else if (t < 0.19) {
      var g1 = ease(0.12, 0.19, t),
        ex = 5 + 4 * g1;
      out.orb = { x: h.x + ex * 0.8, y: h.y, rx: ex, ry: 5 + 1.5 * g1, ring: false };
      out.arcs = 1;
    } else if (t < 0.25) {
      out.orb = { x: h.x + 9 + 8 * ease(0.19, 0.25, t), y: h.y, rx: 8, ry: 8, ring: false };
      out.arcs = 0.6;
    } else if (t < 0.29) {
      out.orb = { x: h.x + 20, y: h.y, rx: 8.5, ry: 8.5, ring: true };
      out.arcs = 0.3;
    } else if (t < 0.36) {
      var g3 = ease(0.32, 0.36, t),
        sx = 6 + 8 * g3;
      out.orb = { x: h.x + sx * 0.85, y: h.y, rx: sx, ry: 4 - 1.5 * g3, ring: false };
      out.arcs = 1 - g3;
    } else if (t < 0.8) {
      var grow = ease(0.36, 0.41, t),
        thin = ease(0.62, 0.7, t);
      out.lance = {
        x: h.x + 2,
        len: THRUST_LANCE * (0.3 + 0.7 * grow),
        thick: 2.2 - 1.1 * thin,
        pulse: 0.1 + 0.75 * ease(0.37, 0.62, t),
        broken: ease(0.66, 0.8, t)
      };
      out.arcs = 0.5 * (1 - ease(0.36, 0.46, t));
    }
    // Scale the light about the saber hand; blade() reads this same geometry for collision.
    var scale = 1.5;
    if (out.orb) {
      out.orb.x = h.x + (out.orb.x - h.x) * scale;
      out.orb.rx *= scale;
      out.orb.ry *= scale;
    }
    if (out.lance) {
      out.lance.x = h.x + (out.lance.x - h.x) * scale;
      out.lance.len *= scale;
      out.lance.thick *= scale;
      out.lance.scale = scale;
    }
    return out;
  }
  ...
  function pixelOrb(ctx, o, hollow) {
    ...
    for (var L = 0; L < layers.length; L++) {
      ...
      for (var dx = -Math.ceil(rx); dx <= Math.ceil(rx); dx++) {
        var q = 1 - (dx / rx) * (dx / rx);
        if (q <= 0) continue;
        var hy = Math.round(ry * Math.sqrt(q));
        ctx.fillRect(cx + dx, cy - hy, 1, hy * 2 + 1);
      }
    }
  }
  ...
  function pixelLance(ctx, l, y, reduced) {
    var x0 = Math.round(l.x),
      n = Math.round(l.len),
      yc = Math.round(y),
      pulse = reduced ? 0.3 : l.pulse;
    for (var i = 0; i <= n; i++) {
      var u = i / Math.max(1, n);
      if (l.broken > 0) {
        if (u < l.broken * 0.5) continue;
        if (i % 9 >= 9 * (1 - 0.8 * l.broken)) continue;
      }
      var half =
        l.thick * Math.min(1, 0.45 + u * 14) * (u > 0.72 ? Math.max(0.18, 1 - (u - 0.72) / 0.28) : 1) +
        1.5 * (l.scale || 1) * Math.exp(-Math.pow((u - pulse) / 0.07, 2)) * (1 - l.broken);
      var hh = Math.round(half),
        x = x0 + i;
      if (hh <= 0) {
        ctx.fillStyle = THRUST_INK[1];
        ctx.fillRect(x, yc, 1, 1);
        continue;
      }
      ctx.fillStyle = THRUST_INK[0];
      ctx.fillRect(x, yc - hh, 1, hh * 2 + 1);
      var e = hh - 1;
      if (hh >= 3) {
        ctx.fillStyle = THRUST_INK[1];
        ctx.fillRect(x, yc - e, 1, e * 2 + 1);
        e--;
      }
      if (e >= 0) {
        ctx.fillStyle = THRUST_INK[2];
        ctx.fillRect(x, yc - e, 1, e * 2 + 1);
        e--;
      }
      if (e >= 0) {
        ctx.fillStyle = THRUST_INK[3];
        ctx.fillRect(x, yc - e, 1, e * 2 + 1);
      }
    }
  }
  ...
  function thrustArcs(ctx, p, fx) {
    var step = Math.floor(p.t * 45),
      h = p.hand,
      s = p.shoulder,
      n = Math.max(1, Math.round(4 * Math.min(1, fx.arcs)));
    for (var k = 0; k < n; k++) {
      var a = fireHash(k * 7.1, step),
        b = fireHash(k * 3.3 + 50, step);
      var x0 = fx.orb ? fx.orb.x + fx.orb.rx * (0.2 * a - 0.4) : h.x + 3,
        y0 = h.y + (b - 0.5) * (fx.orb ? fx.orb.ry * 1.8 : 5);
      var x1 = k < 2 ? s.x - 1 + 5 * b : x0 - 7 - 9 * a,
        y1 = k < 2 ? s.y + (a - 0.5) * 8 : y0 + (b - 0.5) * 13;
      var px = x0,
        py = y0;
      ctx.fillStyle = k % 2 ? THRUST_INK[2] : THRUST_INK[1];
      for (var j = 1; j <= 4; j++) {
        var u = j / 4,
          off = j === 4 ? 0 : (fireHash(k * 11 + j, step) - 0.5) * 5,
          qx = x0 + (x1 - x0) * u,
          qy = y0 + (y1 - y0) * u + off,
          m = Math.max(1, Math.round(Math.hypot(qx - px, qy - py)));
        for (var e = 0; e <= m; e++)
          ctx.fillRect(Math.round(px + ((qx - px) * e) / m), Math.round(py + ((qy - py) * e) / m), 1, 1);
        px = qx;
        py = qy;
      }
    }
  ...
  function drawThrust(ctx, p, reduced) {
    ...
    ctx.save();
    ctx.imageSmoothingEnabled = false;
    if (!reduced && fx.arcs > 0.02) thrustArcs(ctx, p, fx);
    if (fx.orb) pixelOrb(ctx, fx.orb, fx.orb.ring);
    if (fx.lance) {
      pixelLance(ctx, fx.lance, p.hand.y, reduced);
      if (fx.lance.broken < 0.35) pixelOrb(ctx, { x: p.hand.x + 2, y: p.hand.y, rx: 2.6, ry: 2.6 }, false);
    }
    ctx.restore();
  }
  function drawFanCharge(ctx, p, reduced, level, clock) {
    var t = p.t,
      expand = ease(0.1, 0.42, t),
      fade = 1 - ease(0.53, 0.7, t);
    if (t < 0.1 || fade <= 0) return;
    level = level || 1;
    var rx = 8 + (29 + (level - 1) * 3) * expand,
      ry = 5 + (24 + (level - 1) * 2) * expand;
    ctx.save();
    ...
    for (var l = 0; l < 4; l++) {
      ...
      var r = rx - l * 1.7,
        h = ry - l * 1.5;
      for (var x = -Math.ceil(r); x <= r; x++) {
        var y = Math.sqrt(Math.max(0, 1 - (x * x) / (r * r))) * h;
        ctx.fillRect(x, -Math.round(y), 1, Math.round(y));
      }
    }
    ctx.globalAlpha = fade * 0.8;
    ctx.strokeStyle = '#b4e8ff';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.ellipse(0, -1, rx, 4, 0, 0, Math.PI * 2);
    ctx.stroke();
    if (!reduced)
      for (var i = 0; i < 5; i++) {
        var a = Math.PI * (i / 4),
          rr = rx * (0.55 + ((t * 2 + i * 0.2) % 1) * 0.35);
        ctx.fillStyle = '#477dcc';
        ctx.fillRect(Math.round(Math.cos(a) * rr), -Math.round(Math.sin(a) * ry * 0.85), 2, 4);
      }
    ctx.globalAlpha = fade * 0.75;
    ctx.strokeStyle = '#d8f7ff';
    for (var ring = 1; ring < level; ring++) {
      ctx.beginPath();
      ctx.ellipse(
        0,
        -3 - ring * 5,
        rx + ring * 5,
        7 + ring * 2,
        reduced ? 0 : Math.sin((clock || 0) * 5 + ring) * 0.08,
        Math.PI,
        Math.PI * 2
      );
      ctx.stroke();
  ...
  function draw(ctx, o) {
    ...
    function part(mask, a, b, c, d) {
      rig.bonePart(ctx, img, { x: 314, y: 627 }, mask, a, b, c, d);
    }
    ...
    var finishGuard = p.stage === 3 ? ease(0.23, 0.41, p.t) * (1 - ease(0.92, 1, p.t)) : 0;
    ...
    var fling = p.stage === 5 ? 1 - ease(0, 0.42, p.t) : 0;
    var rearElbow = {
    ...
    if (turnReady && p.stage === 2 && p.viewTurn > 0.2) {
      ...
      ctx.save();
      ctx.translate(p.hip.x, p.hip.y);
      ctx.rotate(p.lean * (1 - p.viewTurn) * 0.5);
      ctx.drawImage(
        turnAtlas,
        column * 512,
        510,
    ...
    if (p.stage < 4 && b.visible && (!smear(p) || smear(p).alpha < 0.35)) {
      ctx.save();
      ctx.lineCap = 'round';
      function line(width, color) {
        ...
        ctx.quadraticCurveTo(
          (b.root.x + b.tip.x) * 0.5 + 3 * Math.sin(p.t * 7),
          (b.root.y + b.tip.y) * 0.5 - (p.stage === 2 ? 1.5 : 5) * Math.sin(p.t * Math.PI),
          b.tip.x,
          b.tip.y
        );
    ...
    ctx.fillStyle = '#122d45';
    ctx.fillRect(-4, -2, 6, 4);
    ctx.fillStyle = '#b8eaf1';
  ...
  function segment(stage, phase, build) {
    var q = pose(stage, phase),
      ...
      by = (build && build.y) || 1;
    return {
      root: { x: b.root.x * bx, y: b.root.y * by },
      tip: { x: b.tip.x * bx, y: b.tip.y * by },
      visible: b.visible
    };
  }
  ...
  function sweep(stage, phase, build, samples) {
    ...
    if (!out.length) {
      var now = segment(stage, phase, build);
      if (now.visible) out.push(now);
    }
    return out;
  }
  g.AstraSaberRig = {
