(function (global) {
  ...
  combat.fan = {span:1.02,fireAt:.72,count:7,speed:265,power:4.5,radius:6,life:1.8};
  ...
  NeonGame.prototype._clearMouse=function(){this.mouseInput={};this._shootHeld=false;this._saberHeld=false;if(this.state&&this.state.player){this.state.player.saberCharge=0;this._emit('charge-state',{level:0});}};
  ...
  NeonGame.prototype._fanBurst=function(){
    ...
    for(var i=0;i<F.count;i++){var a=(-165+150*i/(F.count-1))*Math.PI/180;this._spawn(p.x+p.w/2,p.y+p.h-12,Math.cos(a)*F.speed*p.saberFacing,Math.sin(a)*F.speed,'player',false,F.power,{kind:'fan-orb',r:F.radius,life:F.life});}
    this._emit('sound',{name:'thrust'});
  };
