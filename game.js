(function (global) {
  ...
  NeonGame.prototype._risingBites=function(){
    ...
    for(var i=0;i<s.enemies.length;i++){var e=s.enemies[i];if(e.dead||!bites(e))continue;
      e.hp-=this._masteryPower?this._masteryPower(cut,'rising'):cut;e.flash=.12;this._particle(e.x,e.y,'#ffb52e','burst');if(e.hp<=0)this._killEnemy(e);}
    var b=s.boss;if(b&&b.active&&!b.down&&!b.phaseOut&&bites(b)){b.hp=Math.max(0,b.hp-(this._masteryPower?this._masteryPower(cut,'rising'):cut));b.flash=.12;}
  };
  ...
  NeonGame.prototype._damageNearby=function(range,dmg,facing){var s=this.state,p=s.player,attackFacing=facing===undefined?p.facing:facing;var segs=global.AstraCombat.saberSweep(p);function reaches(box,cx,cy,wide){if(segs)return global.AstraCombat.bladeTouches(segs,box,global.AstraCombat.saberPad(p));var dx=cx-p.x;return Math.abs(dx)<range+(wide||0)&&((dx>0?1:-1)===attackFacing)&&Math.abs(cy-(p.y+p.h/2))<(p.saberCombo===6?1.5:1)*(wide?75:55);}for(var i=0;i<s.enemies.length;i++){var e=s.enemies[i];if(!e.dead&&reaches(e,e.x,e.y+e.h/2,0)){e.hp-=this._masteryPower?this._masteryPower(dmg,p.saberCombo===6?'thrust':'saber'):dmg;e.flash=.12;this._particle(e.x,e.y,'#ffb52e','burst');if(e.hp<=0)this._killEnemy(e);}}if(s.boss&&s.boss.active&&!s.boss.down&&!s.boss.phaseOut&&reaches(s.boss,s.boss.x,s.boss.y+s.boss.h/2,s.boss.w*.4)){s.boss.hp=Math.max(0,s.boss.hp-(this._masteryPower?this._masteryPower(dmg,p.saberCombo===6?'thrust':'saber'):dmg));s.boss.flash=.12;}}
