(function (global) {
  ...
  // mark.
  var combat = { normalPower: 1, chargeThreshold: .55 };
  ...
  // row for it.
  combat.saberComboPower = [1, 1, .625, .625];
  ...
  combat.fan = {span:1.02,fireAt:.72,count:7,speed:265,power:4.5,radius:6,life:1.8,chargeMax:1.4,chargeStep:.7,volleyGap:.12};
  combat.fanLevel=function(p){return 1+Math.min(2,Math.floor(((p.fanCharge||0)+1e-8)/combat.fan.chargeStep));};
  ...
  combat.saberChargeShown = function (p) {
    if(p&&p.saberCombo===7&&p.saberTime>0)return p.fanReleased?0:Math.min(1,(p.fanCharge||0)/combat.fan.chargeMax);
  ...
  combat.saberPhase = function(p) {
    ...
    if(p&&(p.saberCombo===4||p.saberCombo===6||p.saberCombo===7))return Math.min(1,elapsed/span);
  ...
  combat.saberHitTimes = [[.16], [.08], [.13, .17, .21, .25]];
  ...
  combat.bossDive = { rise: .45, lift: 150, hover: .55, lock: .18, track: 260, fall: 2600, stuck: 1.0,
                      shockSpeed: 240, shockLife: .75, shockR: 7 };
  ...
  combat.idlePose = function (p) { return !!p && !(p.shootPoseTime > 0) && p.onGround && Math.abs(p.vx || 0) <= 20 && !(p.dashTime > 0) && !(p.saberTime > 0) && !p.wallDir; };
  ...
  combat.idleOffset = function (p) {
    if (!p || p.saberTime > 0 || p.shootPoseTime > 0 ||
  ...
  NeonGame.prototype._bind=function(){
    var self=this;
    this._keydown=function(e){ var k=e.key.toLowerCase(); if(self.state.mode==='playing'&&['arrowleft','arrowright','arrowup','arrowdown','s',' ','shift','z','x','j','k','a','d','p','escape','r'].indexOf(k)>=0)e.preventDefault();
  ...
  NeonGame.prototype._clearMouse=function(){this.mouseInput={};this._shootHeld=false;this._saberHeld=false;if(this.state&&this.state.player){this.state.player.saberCharge=0;if(this.state.player.saberCombo===7&&!this.state.player.fanReleased){this.state.player.saberTime=0;this.state.player.saberCombo=0;this.state.player.fanCharge=0;}this._emit('charge-state',{level:0});}};
  ...
  NeonGame.prototype._mapKey=function(k,v){var m={a:'left',arrowleft:'left',d:'right',arrowright:'right',' ':'jump',arrowup:'up',w:'up',s:'down',arrowdown:'down',z:'jump',shift:'dash',x:'dash',j:'shoot',k:'saber'};if(m[k])this.setInput(m[k],v);};
  ...
  NeonGame.prototype._pollGamepad=function(){this.padInput={};if(typeof navigator==='undefined'||!navigator.getGamepads)return;var g=navigator.getGamepads()[0];if(!g)return;var ax=g.axes&&g.axes[0]||0,ay=g.axes&&g.axes[1]||0;this.padInput.left=!!((g.buttons[14]&&g.buttons[14].pressed)||ax<-.35);this.padInput.right=!!((g.buttons[15]&&g.buttons[15].pressed)||ax>.35);this.padInput.up=!!((g.buttons[12]&&g.buttons[12].pressed)||ay<-.35);this.padInput.down=!!((g.buttons[13]&&g.buttons[13].pressed)||ay>.35);this.padInput.jump=!!(g.buttons[0]&&g.buttons[0].pressed);this.padInput.dash=!!(g.buttons[1]&&g.buttons[1].pressed);this.padInput.shoot=!!(g.buttons[2]&&g.buttons[2].pressed);this.padInput.saber=!!(g.buttons[3]&&g.buttons[3].pressed);var start=!!(g.buttons[9]&&g.buttons[9].pressed);if(start&&!this._padStartLatch){this._padStartLatch=true;if(!this.menuOwnsPad){if(this.state.mode==='playing')this.pause();else if(this.state.mode==='paused')this.resume();}}if(!start)this._padStartLatch=false;};
  NeonGame.prototype._tick=function(dt){var s=this.state,p=s.player;if(s.mode!=='playing')return;s.time+=dt;s.timeElapsed=s.time;s.messageTimer=Math.max(0,s.messageTimer-dt);p.animTime+=dt;p.invuln=Math.max(0,p.invuln-dt);p.dashCooldown=Math.max(0,p.dashCooldown-dt);p.saberTime=Math.max(0,p.saberTime-dt);p.coyote=Math.max(0,p.coyote-dt);p.jumpBuffer=Math.max(0,p.jumpBuffer-dt);p.upBuffer=Math.max(0,(p.upBuffer||0)-dt);p.risingHold=Math.max(0,(p.risingHold||0)-dt);if(p.risingWind>0){p.risingWind-=dt;if(p.risingWind<=0&&p.saberCombo===4){var R=global.AstraCombat.rising;p.vy=-R.lift;p.onGround=false;p.coyote=0;p.risingUp=true;if(Math.abs(p.vx)<R.drift)p.vx=p.saberFacing*R.drift;}}
    ...
    shoot=shoot||this.input.shoot;p.fireCooldown=Math.max(0,p.fireCooldown-dt);p.shootPoseTime=Math.max(0,(p.shootPoseTime||0)-dt);var fireNormal=false;if(shoot)this._shootHeld=true; if(!shoot&&this._shootHeld){fireNormal=p.fireCooldown<=0;if(fireNormal)p.fireCooldown=.18;this._shootHeld=false;} this._emit('charge-state',{level:global.AstraCombat.saberChargeShown(p)}); var saberElapsed=global.AstraCombat.saberSpan(p)-p.saberTime;if(p.saberTime<=0&&p.saberCombo>0){if(p.saberQueued&&p.saberCombo<3){p.saberCombo++;p.saberTime=global.AstraCombat.saberDuration;p.saberHit=false;p.saberHits=0;p.saberQueued=false;p.saberFacing=p.facing;p.attackSerial=(p.attackSerial||0)+1;this._emit('sound',{name:'saber',combo:p.saberCombo});}else{if(p.saberCombo===4&&!p.onGround)p.risingHold=global.AstraCombat.rising.hold;p.saberCombo=0;p.saberQueued=false;}}if(saber&&!this._saberHeld){if(p.saberTime>0){if(p.saberCombo<3&&saberElapsed>=global.AstraCombat.saberComboWindowStart&&saberElapsed<global.AstraCombat.saberDuration)p.saberQueued=true;}else{p.risingHold=0;p.saberCombo=(down||p.downBuffer>0)&&p.onGround?7:wantsRising?4:1;if(p.saberCombo===7){p.dashTime=0;p.vx=0;p.fanCharge=0;p.fanReleased=false;}if(p.saberCombo===4)p.risingSerial=(p.risingSerial||0)+1;p.saberTime=global.AstraCombat.saberSpan(p);if(p.saberCombo===4&&p.onGround)p.risingWind=global.AstraCombat.rising.crouch;p.saberHit=false;p.saberHits=0;p.saberQueued=false;p.saberFacing=p.facing;p.attackSerial=(p.attackSerial||0)+1;this._emit('sound',{name:'saber',combo:p.saberCombo});}}
    if(down&&p.onGround&&p.saberCombo===1&&p.saberTime>0&&!p.saberHits&&saberElapsed<global.AstraCombat.risingConvert){p.saberCombo=7;p.fanCharge=0;p.fanReleased=false;p.dashTime=0;p.vx=0;p.saberQueued=false;p.saberTime=global.AstraCombat.fan.span-saberElapsed;}
    ...
    if(up&&p.saberCombo===1&&p.saberTime>0&&!p.saberHits&&
      ...
      p.saberCombo=4;p.risingSerial=(p.risingSerial||0)+1;p.saberQueued=false;p.saberTime=global.AstraCombat.rising.span-saberElapsed;
    ...
    if(saber){
      if(down||p.saberCombo===4||p.saberCombo===6||p.saberCombo===7||p.risingHold>0)p.saberCharge=0;
      ...
      if((p.saberCharge||0)>=TH.ready&&!(p.saberTime>0)){
        p.risingHold=0;p.saberCombo=6;p.saberTime=TH.span;p.saberHit=false;p.saberHits=0;p.saberQueued=false;p.saberFacing=p.facing;p.attackSerial=(p.attackSerial||0)+1;
    ...
    if(p.saberCombo===7&&p.saberTime>0&&!p.fanReleased){
      ...
      if(saber){
        ...
        p.saberTime=Math.max(p.saberTime,FC.span*.55);
      }else{
        p.fanReleased=true;
  ...
  NeonGame.prototype._saberImpact=function(){var s=this.state,now=s.time||0;if(s.lastSaberImpact!==undefined&&now-s.lastSaberImpact<.075)return;s.lastSaberImpact=now;this._emit('sound',{name:'saber-hit'});};
  NeonGame.prototype._risingBites=function(){
    var s=this.state,p=s.player,C=global.AstraCombat,R=C.rising,segs=C.risingFire(p);
    if(!segs)return;
    var now=R.span-p.saberTime,cut=C.saberPower*C.saberComboPower[3];
    function bites(t){
      if(t.risingSerial!==p.risingSerial){t.risingSerial=p.risingSerial;t.risingBites=0;t.risingBiteAt=-1;}
      if(t.risingBites>=R.bites||(t.risingBites>0&&now-t.risingBiteAt<R.biteEvery-1e-9))return false;
      if(!C.bladeTouches(segs,t,C.risingFirePad))return false;
      t.risingBites++;t.risingBiteAt=now;p.saberHits=(p.saberHits||0)+1;p.saberHit=true;return true;
    }
    for(var i=0;i<s.enemies.length;i++){var e=s.enemies[i];if(e.dead||!bites(e))continue;
      e.hp-=this._masteryPower?this._masteryPower(cut,'rising'):cut;this._saberImpact();e.flash=.12;this._particle(e.x,e.y,'#ffb52e','burst');if(e.hp<=0)this._killEnemy(e);}
    var b=s.boss;if(b&&b.active&&!b.down&&!b.phaseOut&&bites(b)){b.hp=Math.max(0,b.hp-(this._masteryPower?this._masteryPower(cut,'rising'):cut));this._saberImpact();b.flash=.12;}
  };
  ...
  NeonGame.prototype._bullets=function(dt){var s=this.state,p=s.player;for(var i=s.bullets.length-1;i>=0;i--){var b=s.bullets[i];if(b.hold&&b.armIn>0){b.armIn-=dt;continue;}if(b.g)b.vy+=b.g*dt;b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;if(b.turn&&b.life<=b.turn){b.vx=-b.vx;b.turn=0;}if(b.armIn>0)b.armIn-=dt;var remove=b.life<=0||b.x<0||b.x>this.worldWidth||b.y>430||b.y<-140;if(!remove&&b.split&&b.life<=b.split.at){this._bossSplit(b);remove=true;}if(remove&&b.fuse&&b.life<=0)this._mineBursts(b);if(!remove&&b.floorOnly){if(b.y+b.r>=310){remove=true;if(b.fxBoss)this._bossImpact(b.x,310,b.fxBoss,20);}}else if(!remove&&(b.g||b.pops)&&this._shellLands(b))remove=true;if(!remove){if(b.team==='player'){for(var j=0;j<s.enemies.length;j++){var e=s.enemies[j];if(!e.dead&&hit({x:b.x-b.r,y:b.y-b.r,w:b.r*2,h:b.r*2},e)){e.hp-=b.power;e.flash=.1;remove=true;for(var z=0;z<3;z++)this._particle(b.x,b.y,'#ffe36e');if(e.hp<=0)this._killEnemy(e);}}if(s.boss&&s.boss.active&&!s.boss.down&&!s.boss.phaseOut&&hit({x:b.x-b.r,y:b.y-b.r,w:b.r*2,h:b.r*2},s.boss)){s.boss.hp-=b.power;s.boss.flash=.1;remove=true;this._burst(b.x,b.y,'#ff4f9a');}}else if(!b.solid&&this._saberDeflects(b)){remove=true;if(this._registerDeflect)this._registerDeflect();for(var k=0;k<5;k++)this._particle(b.x,b.y,'#80fff0');this._particle(b.x,b.y,'#e5ffff','ring');}else if(!(b.armIn>0)&&!b.harmless&&hit(b.hw?{x:b.x-b.hw,y:b.y-b.hh,w:b.hw*2,h:b.hh*2}:{x:b.x-b.r,y:b.y-b.r,w:b.r*2,h:b.r*2},p)&&p.invuln<=0){p.hp--;if(this._registerHit)this._registerHit(b.source||{label:"敵弾"});p.invuln=.8;if(!b.hw)remove=true;s.shake=.25;this._emit('sound',{name:'hurt'});if(p.hp<=0)this._die();}}if(remove)s.bullets.splice(i,1);}};
  ...
  NeonGame.prototype._bossFire=function(b,name){
    ...
    if(name==='volley'){
      ...
      b.fly=true;b.vy=0;b.leap=false;b.slammed=true;
      b.move={kind:'dive',phase:'rise',t:0,fromY:b.y,lockX:null,done:false};
    } else if(name==='swing'){
      var SW=global.AstraCombat.bossSwing,anchor=global.AstraCombat.bossSwingAnchor(b),
  ...
  NeonGame.prototype._bossMove=function(b,dt){
    var s=this.state,p=s.player,C=global.AstraCombat,arena=C.arena,m=b.move,u,e;
    if(!m||m.done)return;
    m.t+=dt;
    if(m.kind==='dive'){
      var D=C.bossDive,hoverY=b.baseY-D.lift,want=p.x+p.w/2-b.w/2;
      function hunt(){var gap=want-b.x,step=D.track*dt;b.x=clamp(b.x+(gap>0?Math.min(step,gap):Math.max(-step,gap)),arena.bossMin,arena.bossMax);}
      if(m.phase==='rise'){
        u=Math.min(1,m.t/D.rise);e=u*u*(3-2*u);b.y=m.fromY+(hoverY-m.fromY)*e;hunt();
        if(u>=1){m.phase='hover';m.t=0;}
        ...
        if(m.t>=D.hover){if(m.lockX===null)m.lockX=b.x;m.phase='fall';m.t=0;b.vy=0;}
      ...
      if(m.phase==='climb'){
        u=Math.min(1,m.t/S.climb);e=u*u*(3-2*u);
        var tx=m.ax+L*Math.sin(m.th0)-b.w/2,ty=m.ay+L*Math.cos(m.th0)-b.h/2;
        b.x=clamp(m.fromX+(tx-m.fromX)*e,arena.bossMin,arena.bossMax);b.y=Math.min(b.baseY,m.fromY+(ty-m.fromY)*e);
        if(u>=1){m.phase='sweep';m.t=0;}
      }else if(m.phase==='sweep'){
        u=Math.min(1,m.t/S.sweep);m.th=m.th0*Math.cos(Math.PI*u);
        b.x=clamp(m.ax+L*Math.sin(m.th)-b.w/2,arena.bossMin,arena.bossMax);b.y=Math.min(b.baseY,m.ay+L*Math.cos(m.th)-b.h/2);
        // it lets go on the far side and drops; slammed stays set, so the landing throws no slam wave
        if(u>=1){m.phase='drop';m.t=0;b.fly=false;b.vy=0;b.slammed=true;}
      }else if(m.phase==='drop'){
        if(b.y>=b.baseY){
          m.phase='saws';m.t=0;
          var sx=b.x+b.w/2,sy=b.baseY+b.h;
          for(var d=-1;d<=1;d+=2)
            this._spawn(sx+d*b.w*.4,sy-S.sawR-1,d*S.sawSpeed,0,'enemy',false,1,{r:S.sawR,kind:'saw',life:S.sawLife,turn:S.sawTurn,fxBoss:b.id});
  ...
  NeonGame.prototype._boss=function(dt){
    ...
    if(!b.phaseOut&&hit(p,b)&&p.invuln<=0){p.hp-=2;if(this._registerHit)this._registerHit(this._attackSource||{boss:b.id,label:"ボスとの接触"});p.invuln=1;s.shake=.3;if(p.hp<=0)this._die();}
  };
  ...
  NeonGame.prototype._die=function(){var s=this.state;if(s.mode==='dead'||s.mode==='victory')return;this._clearMouse();s.mode='dead';s.player.hp=0;s.player.saberTime=0;s.player.dashTime=0;s.player.saberCharge=0;s.deathFx={time:0,duration:1.15,x:s.player.x+s.player.w/2,y:Math.min(286,s.player.y+s.player.h/2),facing:s.player.facing};s.message='';s.messageTimer=999;s.shake=.5;this._emit('death',this.snapshot());};
