...
P._masteryPower=function(power,kind){var s=this.state,m=s.mastery,b=s.boss,p=s.player;if(!m)return power;
 ...
 if(m.ready>0&&serial!==m.sourceSerial&&kind!=='fan'){
   m.ready=0;m.counterSerial=serial;m.counters++;note(s,'COUNTER x1.5');
 }
 var mult=kind!=='fan'&&m.counterSerial===serial?1.5:1;
 if(b&&kind==='thrust'&&b.attack==='rest'){mult*=1.25;note(s,m.counterSerial===serial?'COUNTER + OPEN x1.875':'OPENING / THRUST x1.25');}
...
var beat=P._bossBeat;P._bossBeat=function(b,def){var pr=this.state.practice;
 if(pr&&pr.move){b.beat=C.bossRoutine(def).findIndex(function(x){return x.move===pr.move;});b.move=null;b.grounded=false;b.fly=!!b.flies;b.phaseOut=false;this._bossWind(b,def,pr.move);return;}
 return beat.call(this,b,def);
};
function enterArena(game){
 if(C.stage.kind!=='gauntlet')return;
 var s=game.state,p=s.player,A=C.arena;
 s.enemies=[];s.pickups=[];p.x=A.spawn+24;p.y=310-p.h;p.onGround=true;s.bossIndex=0;
 game._spawnBoss(C.stage.bosses[0]);s.camera.x=Math.max(0,p.x-160);
}
var start=P.start;P.start=function(options){start.call(this,options);enterArena(this);};
P.startPractice=function(id,move){var def=g.AstraBosses.get(id),valid=C.bossRoutine(def).some(function(x){return x.move===move;});
 this.start({stage:'gauntlet',difficulty:this.difficulty||'normal'});
 var s=this.state,A=C.arena,p=s.player;s.practice={boss:def.id,move:valid?move:null};
 s.enemies=[];s.pickups=[];s.bullets=[];s.particles=[];s.bossIndex=0;
 var b=this._spawnBoss(def.id);p.x=Math.max(A.gate+30,b.x-180);p.y=310-p.h;p.onGround=true;
 s.camera.x=Math.max(0,p.x-200);s.message='PRACTICE / R : RESTART';s.messageTimer=2;
 this._clearMouse();this.input={};this.keys={};this.pressed={};
};
P.restartPractice=function(){var p=this.state.practice;if(p)this.startPractice(p.boss,p.move);};
var retry=P.retry;P.retry=function(){if(this.state.practice){this.restartPractice();return;}if(this.state.mode!=='dead')return;retry.call(this);if(this.state.mode==='playing')enterArena(this);};
// Small pure record update shared by the UI and regression tests. Difficulty is part of the key.
function recordBoss(all,r){if(r.practice)return null;var key=r.difficulty+':'+r.id,old=all[key]||{},previous=old.bestTime;
 var fresh={clears:(old.clears||0)+1,bestTime:previous===undefined?r.time:Math.min(previous,r.time),noDamage:!!old.noDamage||r.hits===0};
 all[key]=fresh;return{record:fresh,delta:previous===undefined?null:r.time-previous};
}
