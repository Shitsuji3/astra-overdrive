...
function guide(){if(modal.hidden)modalReturn=document.activeElement;modal.hidden=false;$('#modal-content').innerHTML='<div class="eyebrow">PILOT MANUAL</div><h2>操作ガイド</h2><table class="guide-table"><tr><th>操作</th><th>キーボード</th><th>ゲームパッド</th></tr><tr><td>移動</td><td>A / D　← / →</td><td>左スティック</td></tr><tr><td>ジャンプ</td><td>Space / Z</td><td>A</td></tr><tr><td>ダッシュ</td><td>Shift / X</td><td>B</td></tr><tr><td>射撃</td><td>J または左クリック（離すと1発）</td><td>X</td></tr><tr><td>セイバー</td><td>K または右クリック</td><td>Y</td></tr><tr><td>溜め突き</td><td>K 長押し→READYで離す</td><td>Y 長押し</td></tr><tr><td>斬り上げ</td><td>↑ / W ＋ セイバー</td><td>上 ＋ Y</td></tr><tr><td>扇状弾（地上）</td><td>↓ / S ＋ セイバー長押し→離す（最大3段）</td><td>下 ＋ Y 長押し→離す</td></tr><tr><td>壁蹴り</td><td>壁につかまり＋ジャンプ</td><td>壁際＋A</td></tr></table><p>弾を斬ると1.5秒間COUNTER READY。次の近接攻撃が1.5倍。ボスの隙に溜め突き、空中の敵に斬り上げで各1.25倍。扇状弾は上方の広範囲へ。</p><p>ESC / P / Start　ポーズ</p><p>メニュー：十字キー / 左スティックで選択、A で決定、B で戻る（設定のスライダーは左右）</p>'}
function settingsModal(){if(modal.hidden)modalReturn=document.activeElement;modal.hidden=false;var m=settings.music===undefined?.45:settings.music,s=settings.sfx===undefined?.7:settings.sfx;$('#modal-content').innerHTML='<div class="eyebrow">SYSTEM CONFIG</div><h2>システム設定</h2><label class="settings-row">BGM <input id="music" type="range" min="0" max="100" value="'+Math.round(m*100)+'"></label><label class="settings-row">SE <input id="sfx" type="range" min="0" max="100" value="'+Math.round(s*100)+'"></label><label class="settings-row">消音 <input id="mute" type="checkbox" '+(settings.muted?'checked':'')+'></label><label class="settings-row">やさしい難易度 <input id="easy" type="checkbox" '+(settings.easy?'checked':'')+'></label><label class="settings-row">画面の揺れを抑える <input id="motion" type="checkbox" '+(settings.motion?'checked':'')+'></label><button data-action="close">戻る</button>';['music','sfx','mute','easy','motion'].forEach(function(id){$('#'+id).onchange=function(){settings.music=+$('#music').value/100;settings.sfx=+$('#sfx').value/100;settings.muted=$('#mute').checked;settings.easy=$('#easy').checked;settings.motion=$('#motion').checked;save();AstraAudio.setMusic(settings.music);AstraAudio.setSfx(settings.sfx);AstraAudio.setMuted(settings.muted);if(game)game.setOptions({reducedMotion:settings.motion})}})}
function helpMenu(){
 if(modal.hidden)modalReturn=document.activeElement;modalView='help';modal.hidden=false;
 $('#modal-content').innerHTML='<h2>設定・操作方法</h2><div class="training-grid"><button data-action="guide">操作方法</button><button data-action="settings">音量・ゲーム設定</button></div>';
 $('#modal-content button').focus();
}
function closeModal(){if(modalView==='help-child'){helpMenu();return;}if(modalView==='practice-boss'){practiceMenu();return;}modalView='';modal.hidden=true;if(modalReturn&&modalReturn.isConnected)modalReturn.focus();else if(game){var b=$('#overlay-actions button');if(b)b.focus()}}
...
function showArmed(){var el=$('#current-stage');if(el)el.textContent='8体のボスに挑む連戦アクション';}
...
function openPicker(){pickerFromPractice=modalView==='practice';modal.hidden=true;modalView='';title.hidden=true;picker.hidden=false;buildPicker()}
...
function closePicker(){if(nodes[pick]){settings.stage=nodes[pick].def.id;save()}picker.hidden=true;title.hidden=false;showArmed();if(pickerFromPractice){pickerFromPractice=false;active(1);practiceMenu();}else active(0)}
...
function practiceMenu(id){
 ...
 if(modal.hidden)modalReturn=document.activeElement;modal.hidden=false;
 var list=AstraBosses.list,body='<h2>練習する</h2><p>ボス練習は本番の記録に入りません。Rで再戦できます。</p>';
 if(!id){body+='<div class="training-grid">'+list.map(function(b){return '<button data-action="practice-boss" data-boss="'+b.id+'">'+b.name+'</button>';}).join('')+'</div><button data-action="stage-select">ステージで練習する</button>';}
...
else if(a==='continue')start();else if(a==='stage-select'){openPicker();AstraAudio.sound('select')}else if(a==='select-back'){closePicker();AstraAudio.sound('move')}else if(a==='launch-stage'){pickerFromPractice=false;settings.stage=b.dataset.stage;save();picker.hidden=true;showArmed();start()}else if(a==='guide'){modalView=modalView==='help'?'help-child':'';guide()}else if(a==='settings'){modalView=modalView==='help'?'help-child':'';settingsModal()}else if(a==='close'){closeModal()}else if(a==='resume'){overlay.hidden=true;AstraAudio.setPaused(false);game.resume()}else if(a==='retry'){overlay.hidden=true;AstraAudio.setPaused(false);game.retry()}else if(a==='replay'){start()}else if(a==='fullscreen'&&document.documentElement.requestFullscreen){var full=document.documentElement.requestFullscreen();if(full&&full.catch)full.catch(function(){})}else if(a==='title'){AstraAudio.startTitle();overlay.hidden=true;modal.hidden=true;if(game)game.toTitle();pickerFromPractice=false;modalView='';shell.hidden=true;picker.hidden=true;title.hidden=false;showArmed();active(0)}});
...
AstraAudio.setMusic(settings.music===undefined?.45:settings.music);AstraAudio.setSfx(settings.sfx===undefined?.7:settings.sfx);AstraAudio.setMuted(!!settings.muted);setInterval(function(){if(game&&game.state)hud(game.state)},250);showArmed();active(0);AstraAudio.startTitle();
})();
